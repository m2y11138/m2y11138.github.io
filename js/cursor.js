...
class CrystalCursor {
  ...
  activateTrailParticle() {
    ...
    if (particle && this.pos.prev) {
      ...
      if (this.angle) {
        ...
        particle.el.style.transform = `translate(-50%, -50%) rotate(${
  ...
  createCrystalBurst(x, y) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const animate = (startTime) => {
        ...
        // 更新粒子样式
        crystal.style.transform = `translate(${currentX - x}px, ${
          currentY - y
        }px) rotate(${rotation + progress * 360}deg) scale(${scale})`;
  ...
  render() {
    if (this.pos.prev && this.pos.curr) {
      ...
      this.cursor.querySelector(
        ".ring-1"
      ).style.transform = `translate(-50%, -50%) scale(${pulse})`;
      this.cursor.querySelector(
        ".ring-2"
      ).style.transform = `translate(-50%, -50%) scale(${pulse * 1.1})`;
      this.cursor.querySelector(
        ".ring-3"
      ).style.transform = `translate(-50%, -50%) scale(${pulse * 1.2})`;
    } else {
